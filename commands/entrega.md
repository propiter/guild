...
Corré los tres oficios, en orden estricto, delegando cada uno y pasando los artefactos hacia
adelante por disco:
...
3. **puerta de entrega** — `purser`: verifica que está listo para el cliente — README vs realidad,
   `.env.example` completo sin secretos, `.gitignore` sano, build/lint/test verdes, y la prueba
   dura: arranca desde un clone/checkout limpio de verdad. Firma o rechaza con la lista exacta de
   lo que falta.
...
- Fase 3: ¿la prueba fue desde un clone limpio de verdad, o desde el directorio de trabajo ya
  usado?
...
las olas que corrió `finisher` con su verificación antes/después, y —si `purser` rechazó— la lista
